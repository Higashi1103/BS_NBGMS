    ...
    $(function () {
        ...
        $('#gender').blur(function () {
            ...
            if (gender === '') {
                $('#genderError').text('性別を入力してください').css('color', 'red');
                $('#gender').addClass('has-error').removeClass('has-success');
                return;
            }
            if (gender !== '男' && gender !== '女') {
                $('#genderError').text('「男」または「女」を入力してください').css('color', 'red');
                $('#gender').addClass('has-error').removeClass('has-success');
            } else {
        ...
        // ==================== クラス内容チェック ====================
        $('#kurasuInput').blur(function () {
            $('#kurasuError').text('');
            const kurasuId = $('#kurasuId').val().trim();
            if (kurasuId === '') {
                $('#kurasuError').text('クラスを選択してください').css('color', 'red');
                $('#kurasuInput').addClass('has-error').removeClass('has-success');
            } else {
                $('#kurasuError').text('✓').css('color', 'green');
                $('#kurasuInput').addClass('has-success').removeClass('has-error');
            }
        });
        // 实时输入验证 内容无误就清楚红色提示
        $('#kurasuInput').on('change input', function () {
            const max = 10;
            const count = $(this).val().length;
            $('#kurasuCount').text('<' + count + '/' + max + '>');
        ...
        $('#other').blur(function () {
            ...
            if (other.length > 200) {
                $('#otherError').text('200文字以内で入力してください').css('color', 'red');
        ...
        $('#other').on('input', function () {
            const max = 200;                          // ★ 与 maxlength 保持一致
            const count = $(this).val().length;       // ★ 当前字数
